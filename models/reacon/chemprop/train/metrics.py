...
# ------------------------------------------------------------
def top3(targets: List[int], preds: Union[List[float], List[List[float]]], threshold: float = 0.5) -> float:
    """
    Top 3 accuracy for multiclass prediction task.
    This is added by wzh
    so it is just a test, used only in multiclass prediction task
    """
    hard_preds = []
    for i in range(len(preds)):
        newlist = sorted(preds[i], reverse=True)[:3]
        for j in newlist:
            if preds[i].index(j) == targets[i]:
                hard_preds.append(preds[i].index(j))
                break
        else:
            hard_preds.append(preds[i].index(newlist[0]))
