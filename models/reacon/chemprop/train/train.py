...
def train(
    ...
    if model.is_atom_bond_targets:
        ...
        epoch_loss_sum, epoch_iter_count = [0]*(len(args.atom_targets) + len(args.bond_targets)), 0
    else:
        loss_sum = iter_count = 0
        epoch_loss_sum = epoch_iter_count = 0

    for batch in tqdm(data_loader, total=len(data_loader), leave=False, miniters=100, mininterval=0):
        # Prepare batch
        ...
        if model.is_atom_bond_targets:
            ...
            epoch_loss_sum = [x + y for x, y in zip(epoch_loss_sum, loss_multi_task)]
            iter_count += 1
            epoch_iter_count += 1
    ...
    if model.is_atom_bond_targets:
        epoch_train_loss = sum(epoch_loss_sum) / epoch_iter_count if epoch_iter_count > 0 else float('nan')
    else:
        epoch_train_loss = epoch_loss_sum / epoch_iter_count if epoch_iter_count > 0 else float('nan')

    return n_iter, epoch_train_loss
