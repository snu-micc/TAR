...
def run_training(args: TrainArgs,
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            debug(f'Epoch {epoch} | Train loss = {epoch_train_loss:.6f}')
            for metric, scores in val_scores.items():
                # Average validation score\
                mean_val_score = multitask_mean(scores, metric=metric)
                debug(f'Epoch {epoch} | Validation {metric} = {mean_val_score:.6f}')
            ...
            log_rows.append({'epoch': epoch, 'train_loss': round(epoch_train_loss, 6),
                             f'val_{args.metric}': round(mean_val_score, 6),
                             'best_val': round(best_score, 6), 'best_epoch': best_epoch})
        ...
        info(f'Training log saved -> {os.path.join(save_dir, "train_log.csv")}')
